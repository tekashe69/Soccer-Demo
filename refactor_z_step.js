...
const C = {
  ...
  BALL_R: 4,
  PLAYER_R: 8,
  FRICTION: 0.96, // Increased friction so ball stops sooner, less out of bounds
...
class Ball {
  ...
  draw() {
    ...
    g.fillStyle(0x000000, 0.25 * shadowScale);
    g.fillEllipse(this.pos.x + 2, this.pos.y + 2, C.BALL_R * 2 * shadowScale, C.BALL_R * 1.2 * shadowScale);
  ...
  update(engine) {
    ...
    if (ball.owner === this) {
      ...
      if (distToOppGoal < 300 && Math.random() < shootThreshold) {
        ...
        // Tuned down shoot power so it's not a laser beam
        let power = 5 + (this.attr.finishing / 20) * 4 + (this.attr.strength / 20) * 2; 

        let shootVec = new Vector(oppGoalX - this.pos.x, goalY + dy - this.pos.y).normalize().mult(power);
        ball.owner = null;
        ball.vel = shootVec;
        
        // 30% chance for a dipping/lob shot
        if (Math.random() < 0.3 && distToOppGoal > 100) {
      ...
      if (Math.random() < passFreq) {
        ...
        if (bestTarget) {
          ...
          // Tuned passing speed to be realistic (slower overall)
          // Short passes: speed 4-6. Long passes: speed 7-9.
          ...
          // Introduce Z element (Lob pass / Through ball in the air)
          // If distance is long (>150) and vision is good, there's a chance to lob it
          if (passLen > 150 && Math.random() < (this.attr.vision / 20)) {
            // Lob pass: calculate Z velocity to land near the target
            // Time to reach target = passLen / power
            let flightTime = passLen / power;
            // From physics: t = 2 * vz / g => vz = t * g / 2
      ...
      // Pick up loose ball if it's low enough
      ...
      if (this.role === 'GK') {
        ...
        if (distToBallGround < C.TACKLE_RANGE && this.cooldown <= 0) {
          if (Math.random() < 0.20) {
      ...
      if (this.role === 'GK') {
        ...
        // Intercept where ball is going
        let interceptDist = distToBallGround;
        target = ball.pos.copy().add(ball.vel.copy().mult(interceptDist / (C.MAX_SPEED*2)));
      } else {
        target = this.basePos.copy();
      }
    }
    ...
    if (this.role === 'GK' && ball.owner === null && distToBallGround < 40 && ball.vel.mag() > 2 && ball.z < 40) {
  ...
  executeSetPiece() {
    ...
    if (type === 'THROW_IN') {
      ...
      if (target) {
        ...
        this.ball.vel = passVec.normalize().mult(4); // Throw in is slow
        this.ball.vz = 1; // Slight arc
      ...
      this.ball.vel = passVec.mult(8); // Crossing speed
      this.ball.vz = 4 + Math.random()*2; // High arc crossing
      ...
      if (type === 'FREE_KICK' && distToGoal < 220 && Math.random() < 0.6) {
         ...
         if (target) {
            ...
            if (type === 'GOAL_KICK') this.ball.vz = 4; // High goal kick
            logEvent(\\\`👟 #\\\${taker.number} takes the \\\${type.replace('_',' ')}.\\\`, 'action');
         } else {
            this.ball.vel = new Vector(taker.team===0?1:-1, 0).mult(6);
         }
      }
    }
...
`;
fs.writeFileSync('refactor_z.js', content, 'utf8');

// Also write a runner to apply the fix
const runner = `
const fs = require('fs');
...
`;
fs.writeFileSync('run_z.js', runner, 'utf8');
